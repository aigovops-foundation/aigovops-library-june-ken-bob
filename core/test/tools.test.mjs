...
async function brokeredToken(c, scope) {
  const { pendingId } = await c.propose(`deploy via ${scope}`, { actor: 'agent:test' });
  const res = await c.decide(pendingId, 'approve', { scope });
  return res.grant.token;
}

test('registry lists vetted tools with their declared envelope', async () => {
...
test('buildToolCode injects input as a sandbox-safe constant', async () => {
...
test('a laptop-safe tool runs with a scope-matched token and emits a receipt', async () => {
  const c = core();
  const token = await brokeredToken(c, 'demo');
...
test('scope mismatch fails closed (token scope != tool requiredScope)', async () => {
  const c = core();
  const token = await brokeredToken(c, 'github-deploy');   // wrong scope for the demo tool
...
test('a kernel-only tool fails closed on the laptop ProcessSandbox', async () => {
  const c = core();
  const token = await brokeredToken(c, 'github-deploy');
