...
test('proposes a file change as a diff + receipt; main tree untouched, nothing committed', () => {
...
test('end-to-end through the governed loop: propose -> approve -> author change, receipt trail verifies', () => {
  ...
  const { pendingId, requiresHumanGate } = core.propose('publish a self-hosted changelog file', { actor: 'agent:self-host' });
  ...
  const change = core.proposeFileChange({ token: decided.grant.token, relPath: 'CHANGELOG.md', content: '# Changelog\n- governed self-hosted change\n' });
  ...
  const p2 = core.propose('publish another file', { actor: 'agent:self-host' });
  const d2 = core.decide(p2.pendingId, 'approve', { scope: 'github-deploy' });
  assert.throws(() => core.proposeFileChange({ token: d2.grant.token, relPath: 'x.md', content: 'x' }), /needs scope 'self-host'/);
});
