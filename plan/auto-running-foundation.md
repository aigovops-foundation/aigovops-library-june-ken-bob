...
| 14 | M3 | products | **Done 2026-08-23.** All four served products are in the trunk: vendor-rfi (aigovops#2), lantern (#4), umbrella (#5), beacon (#6). `git filter-repo --to-subdirectory-filter` then merge — NOT `git subtree add`, and NOT squash-merged, either of which flattens the history the move exists to keep. 122 commits traverse; trunk 488 KiB → 5.6 MiB | done |
| 14c | M3 | `aigovops` | **The trunk's gate does not test products** (aigovops#3). Scoped to the declared workspaces, because a product's tests need the product's own dependencies. Each product's own CI is still its gate. A per-product lane in the trunk is unbuilt work | Yellow |
| 14d | M3 | `aigovops-beacon` | **Beacon's five media files are not in the trunk copy** — 190 MiB of them, stripped at 1 MiB. ~20 references are dead in the trunk copy and live on beacon's own Pages. Move them to a Release before the trunk ever serves beacon's `docs/` | Yellow |
