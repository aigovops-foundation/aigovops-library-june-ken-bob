...
| 11a | Adopt the five repo-scoped reusables in the trunk | Yellow | **done, and the row was wrong.** It proposed a cross-repo PAT — the exact design `reusable-red-main-watchdog`'s header had already rejected in writing. Calling a reusable across repos runs the job in the *caller's* context with the caller's own token; nothing crosses. The trunk adopted all five in one line each, **no new credential**: aigovops#7 |
...
| 12b | Receipts for skill runs, not just scheduled runs | Yellow | **done** — the runner emitted only on success: 5 invocations left 1 receipt, and a **refused red skill left none**. Now every run leaves exactly one, carrying outcome and declared risk, with no input or error text |
| 23 | Clear 28 merged branches, and stop them accumulating | **Red** | **Bob or Ken** — two clicks. `repo-sync-check` has been red since 2026-08-17 and is **working correctly**: it found merged branches never deleted, 2 then, **28 now** — 26 of them from this session's own PRs. **Zero stranded work**: every live branch has a merged PR. Delete them, then turn on *Settings → General → Automatically delete head branches*. Using the lane's `ignore_branches` here would be faking green |
| 12c | Key the blocking gate on the declared class, not a skill name | Yellow | **done** — enforcement now refuses any skill declaring `risk: red`, before either dispatch path. Proven with a *new* red skill whose backend genuinely works: under the old gate it would have executed. `--approve` does not unlock red |
