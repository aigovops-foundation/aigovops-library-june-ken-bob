...
| 12c | The blocking gate is hardcoded to one skill name | **Red** | **Bob or Ken** — enforcement reads `handlerKey === 'op-github-deploy'`, so a *second* red skill given a handler would not be stopped. `estate-mailer` is red with no handler, which is the only reason this is theoretical. Changing enforcement is a founder's call, not a side effect of adding receipts |
